...
const computeWordCount = async () => {
  const { files } = getPendingUpload()
  ...
  docWordCount.value = total
}

const BASE_TOKENS         = 12_000
const PER_AGENT_SETUP     = 3_500
const PER_AGENT_PER_ROUND = 220
const REPORT_TOKENS       = 18_000
const WORDS_TO_TOKENS     = 1.33

const tokenEstimate = computed(() => {
  const agents = profiles.value.length
  const rounds = useCustomRounds.value ? customMaxRounds.value : (autoGeneratedRounds.value || 0)
  if (agents === 0 || rounds === 0) return null
  const docTokens   = Math.round(docWordCount.value * WORDS_TO_TOKENS)
  const base        = BASE_TOKENS + docTokens
  const agentSetup  = PER_AGENT_SETUP * agents
  const simActivity = PER_AGENT_PER_ROUND * agents * rounds
  const report      = REPORT_TOKENS
  return { total: base + agentSetup + simActivity + report, base, agentSetup, simActivity, report, agents, rounds, wordCount: docWordCount.value }
})
...
.token-estimate-card { margin: 24px 0 16px; border: 1px solid #dbeafe; background: #eff6ff; border-radius: 14px; overflow: hidden; }
.te-header { display: flex; align-items: center; gap: 8px; padding: 12px 18px; border-bottom: 1px solid #dbeafe; font-weight: 700; font-size: 0.82rem; color: #1d4ed8; }
.te-badge { margin-left: auto; background: #bfdbfe; color: #1d4ed8; padding: 2px 8px; border-radius: 20px; font-size: 0.68rem; font-weight: 600; }
.te-body { padding: 14px 18px; }
.te-total { font-size: 1.5rem; font-weight: 800; color: #1e40af; letter-spacing: -0.5px; margin-bottom: 12px; }
.te-breakdown { display: flex; flex-direction: column; gap: 6px; margin-bottom: 10px; }
.te-row { display: flex; justify-content: space-between; font-size: 0.8rem; color: #374151; }
.te-hint { font-size: 0.72rem; color: #6b7280; font-family: 'DM Mono', monospace; padding-top: 10px; border-top: 1px solid #dbeafe; }
</style>