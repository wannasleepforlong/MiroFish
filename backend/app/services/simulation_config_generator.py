...
class PlatformConfig:
    ...
    # Viral threshold (triggers spread after reaching this many interactions)
...
class SimulationConfigGenerator:
    ...
    Adopts step-by-step generation strategy:
    ...
    def __init__(
        ...
        # Using LLMClient automatically handles multiple providers if configured
        self.llm = LLMClient(
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            try:
                # LLMClient.chat provides rotation and thinking-tag removal
...
- reasoning (string): Brief explanation of configuration"""
            ...
            prompt = f"""Based on the following simulation requirements, generate time simulation configuration. Respond in Chinese.
...
### Basic Principles (for reference, adjust based on specific events and participant groups):
- Target users follow Beijing time daily habits
- Late night 0-5am: almost no activity (activity coefficient 0.05)
- Morning 6-8am: gradually increasing (activity coefficient 0.4)
- Work hours 9-18: moderate activity (activity coefficient 0.7)
- Evening 19-22: peak hours (activity coefficient 1.5)
- After 23: declining activity (activity coefficient 0.5)
- General pattern: low in early morning, increasing in morning, moderate during work hours, peak in evening
- **Important**: Adjust time periods based on event nature and participant characteristics
  - E.g.: Student groups may peak at 21-23; media active all day; official institutions only during work hours
  - E.g.: Breaking news may cause late-night discussion, off_peak_hours can be shortened accordingly
...
{{
    ...
    "reasoning": "<brief explanation>"
            ...
            prompt = f"""Based on the following simulation requirements, generate event configuration. Respond in Chinese.
...
{{
    "agent_configs": [
        ...
        ...
            ...
            prompt = f"""Based on the following information, generate social media activity configuration for each entity. Respond in Chinese.
...
## Task
Generate activity configuration for each entity. Guidelines:
- **Time should match Chinese daily habits**: almost no activity 0-5am, most active 19-22 evening
- **Official institutions** (University/GovernmentAgency): Low activity (0.1-0.3), work hours (9-17), slow response (60-240 min), high influence (2.5-3.0)
- **Media** (MediaOutlet): Medium activity (0.4-0.6), all day (8-23), fast response (5-30 min), high influence (2.0-2.5)
- **Individuals** (Student/Person/Alumni): High activity (0.6-0.9), mainly evening (18-23), fast response (1-15 min), low influence (0.8-1.2)
- **Public figures/Experts**: Medium activity (0.4-0.6), medium-high influence (1.5-2.0)
...
{{
    "agent_configs": [
        {{
            ...
            "posts_per_hour": <posting frequency>,
            "comments_per_hour": <comment frequency>,
            "active_hours": [<list of active hours, considering Chinese daily habits>],
            "response_delay_min": <min response delay in minutes>,
            "response_delay_max": <max response delay in minutes>,
            "sentiment_bias": <-1.0 to 1.0>,
            "stance": "<supportive/opposing/neutral/observer>",
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent configuration using rules (Chinese daily habits)"""
