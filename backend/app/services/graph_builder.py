...
class GraphBuilderService:
    ...
    def create_graph(self, name: str, max_retries: int = 3) -> str:
        """Create Zep graph (public method with retry mechanism)"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"Graph creation failed (attempt {attempt + 1}/{max_retries}): {str(e)}")
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any], max_retries: int = 3):
        """Set graph ontology (public method with retry mechanism)"""
        import warnings
        ...
        # Call Zep API to set ontology (with retry)
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(f"Set ontology failed (attempt {attempt + 1}/{max_retries}): {str(e)}")
                        print(f"Retrying in {wait_time} seconds...")
                        time.sleep(wait_time)
                    else:
                        raise
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        if progress_callback:
                            progress_callback(
                                f"Batch {batch_num} failed, retrying in {wait_time}s ({attempt + 1}/{max_retries})...",
    ...
    def _generate_smart_news_queries(self, query: str, context_text: Optional[str] = None, k: int = 3) -> List[str]:
        ...
        try:
            ...
            system_prompt = "You are an expert at optimizing search queries for the NewsAPI (v2). NewsAPI search works best with concise keywords and simple phrases. It often returns zero results for full sentences or descriptive natural language. Respond ONLY with a JSON array of strings."
            user_prompt = f"""Based on the following simulation requirement and document context, generate exactly {k} distinct, keyword-focused NewsAPI search queries (3-6 words each) that will find relevant live news to contextualize this simulation.

DO NOT use full sentences or conversational language.
DO NOT use filler phrases like "public opinion on", "current status of", or "information about".
Focus on the core entities and topics.
...
Example format: ["EU AI Act compliance", "generative AI regulation Europe", "AI governance public sentiment"]"""
        ...
        try:
            ...
            for s_query in search_queries:
                try:
                    ...
                    response = fetcher.fetch_recent_news(s_query, hours=168, sort_by="popularity", page_size=limit)
                    # response = fetcher.fetch_top_headlines(query=s_query, page_size=limit)
                    articles = response.get('articles', [])
                    print(f"DEBUG: fetch_and_add_news - query '{s_query}' found {len(articles)} articles")
